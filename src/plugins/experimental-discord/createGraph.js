...
export type ChannelWeightMap = {[ref: string]: NodeWeight};
  ...
  for (const channel of channels) {
    ...
    for (const message of messages) {
      ...
      for (const reaction of reactions) {
        ...
        // get the weight for the given channel
        const channelWeight = channelWeights.default || 1;

        const node = reactionNode(reaction, message.timestampMs, guild);
        if (reactionWeight != null) {
          wg.weights.nodeWeights.set(
            node.address,
            roleWeight * reactionWeight * channelWeight
          );
        }
